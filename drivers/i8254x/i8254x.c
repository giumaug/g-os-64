...
static void rx_init_i8254x(t_i8254x* i8254x)
{
	...
	for (i=0;i<NUM_RX_DESC;i++)
	{
		...
		rx_desc[i].hi_addr = FROM_VIRT_TO_PHY(U_64_HIGH(data_buffer));
		rx_desc[i].low_addr = FROM_VIRT_TO_PHY(U_64_LOW(data_buffer));
		rx_desc[i].status=0;
	...
	write_i8254x(i8254x,RDBAL_REG,FROM_VIRT_TO_PHY(U_64_LOW(rx_desc)));
	write_i8254x(i8254x,RDBAH_REG,FROM_VIRT_TO_PHY(U_64_HIGH(rx_desc)));
...
static void tx_init_i8254x(t_i8254x* i8254x)
{
	...
	write_i8254x(i8254x,TDBAL_REG,FROM_VIRT_TO_PHY(U_64_LOW(tx_desc))); //first 4 bit zero
	write_i8254x(i8254x,TDBAH_REG,FROM_VIRT_TO_PHY(U_64_HIGH(tx_desc)));
...
void int_handler_i8254x()
{
	...
	else if (status & ICR_RXT0)
	{
		...
		while(rx_desc[cur].status & 0x1)
		{
			...
			data_buffer = kmalloc(MTU_ETH);
			rx_desc[cur].hi_addr = FROM_VIRT_TO_PHY(U_64_HIGH(data_buffer));
			rx_desc[cur].low_addr = FROM_VIRT_TO_PHY(U_64_LOW(data_buffer));
...
void send_packet_i8254x(t_i8254x* i8254x,void* frame_addr,u16 frame_len)
{
	...
	tx_desc[cur].low_addr = FROM_VIRT_TO_PHY(U_64_LOW(phy_frame_addr));
	tx_desc[cur].hi_addr = FROM_VIRT_TO_PHY(U_64_HIGH(phy_frame_addr));
