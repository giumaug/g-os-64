...
int _bind(int sockfd,u32 src_ip,u32 src_port,u32 dst_ip,u16 dst_port)
{
	...
	if (socket != NULL)
	{
		if (socket->type == 2)
		{
			...
			{
				SPINLOCK_LOCK(system.network_desc->udp_desc->lock,GET_CPU_INDEX);
				hashtable_put(system.network_desc->udp_desc->conn_map,src_port,socket);
				SPINLOCK_UNLOCK(system.network_desc->udp_desc->lock,GET_CPU_INDEX);
				ret = 0;
			}
...
t_hashtable* clone_socket_desc(t_hashtable* socket_desc)
{
