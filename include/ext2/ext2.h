...
#define READ_BYTE(src, dst) do { unsigned char* p; p = (unsigned char*)src; dst = ( *p ); } while(0);
#define READ_WORD(src, dst) do { unsigned short* p; p = (unsigned short*)src; dst = *p; } while(0);
#define READ_DWORD(src, dst) do { unsigned int* p; p = (unsigned int*)src; dst = *p; } while(0);

#define WRITE_BYTE(src, dst) do { unsigned char* p; p = (unsigned char*)dst; *p = src; } while(0);
#define WRITE_WORD(src, dst) do { unsigned short* p; p = (unsigned short*)dst; *p = src; } while(0);
#define WRITE_DWORD(src, dst) do { unsigned int* p; p = (unsigned int*)dst; *p = src; } while(0);
